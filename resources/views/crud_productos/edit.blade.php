...
<div style="padding-left: 5%;">
    <h1 class="text-black">Editar earn_disponible</h1>

    <form action="{{ route('crud_productos.update', $earn_disponible) }}" method="POST" class="text-black">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="currency" id="currency" required class="px-3 py-2 border rounded-lg">
                @foreach ($coins as $coin)
                    <option value="{{ $coin->symbol }}" {{ $coin->symbol == $earn_disponible->currency ? 'selected' : '' }}>{{ $coin->symbol }}</option>
                @endforeach
            </select>
        </div> 

        <label for="APR">APR:</label><br>
        <input type="number" name="APR" id="APR" step="0.01" required class="border rounded-md p-2" value="{{ number_format($earn_disponible->APR, 2) }}">
        <br><br>

        <label for="APR">Duración (días):</label><br>
        <input type="number" step="1" name="duracion" id="duracion" required class="border rounded-md p-2" value="{{ number_format($earn_disponible->duracion, 0) }}">
        <br><br>


        <label for="APR">Cantidad disponible:</label><br>
        <input step="0.00000001" type="number" name="cantidad_disponible" id="cantidad_disponible" required class="border rounded-md p-2" value="{{ $earn_disponible->cantidad_disponible }}">
        <br><br>

        <label for="APR">Máximo usuario:</label><br>
        <input step="0.00000001" type="number" name="maximo_usuario" id="maximo_usuario" required class="border rounded-md p-2" value="{{ $earn_disponible->maximo_usuario }}">
        <br><br>

        <label for="APR">Mínimo usuario:</label><br>
        <input type="number" step="0.00000001" name="minimo_usuario" id="minimo_usuario" required class="border rounded-md p-2" value="{{ number_format($earn_disponible->minimo_usuario, 8) }}">
        <br><br>
