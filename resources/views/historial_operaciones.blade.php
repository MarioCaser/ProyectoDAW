...
@section('content')
    ...
    <div class="bg-gray-100 w-full flex items-center justify-between">
        ...
        <a href="{{ route('historial_operaciones') }}" class="flex-1 text-gray-800 hover:bg-gray-300 hover:text-gray-800 px-3 py-3 rounded-md text-sm font-bold text-center">Historial de Operaciones</a>
    </div>
                ...
                @foreach($ordenes as $orden)
                    @if($orden->resultado == "completada")
                        <tr class="highlight-row">
                            ...
                            <td class="py-2 px-4 text-center hidden lg:table-cell ">{{ $orden->resultado }}</td>
                            <td class="py-2 px-4 text-center">{{ $orden->created_at }}</td>
